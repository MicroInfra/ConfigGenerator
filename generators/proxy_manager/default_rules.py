...
class Addon:
    ...
    def response(self, flow: HTTPFlow):
        ...
        thread = Thread(
            target=send_metrics, args=(flow, start, specific_metrics)
        )
        thread.start()


def send_metrics(
    flow: HTTPFlow, start_time, specific_metrics: list or None = None
):
    import requests
    ...
    metrics.append(
        format_metric(
            "response_code", 1, "counter", {"code": flow.response.status_code}
        )
    )
    ...
    metrics.append(
        format_metric("response_size", len(flow.response.get_text()), "sum")
    )
    metrics.append(
        format_metric("request_size", len(flow.request.get_text()), "sum")
    )
...
def format_metric(name, value, type="gauge", labels=None):
    if labels:
        return {
            "name": SERVICE_NAME + "_" + name,
            "value": value,
            "type": type,
            "labels": labels,
        }
    return {"name": SERVICE_NAME + "_" + name, "value": value, "type": type}
